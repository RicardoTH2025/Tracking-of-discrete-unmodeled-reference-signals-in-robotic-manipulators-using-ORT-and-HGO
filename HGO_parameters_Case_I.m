...
FS=14; %tamaño de fuente para las gráficas
 ...
 t1_op=deg2rad(85);  %theta 1 o x1 de operación
 t2_op=deg2rad(-109);   %theta 2 o x3 de operación

 Ts=0.001;    %en el paper 0.0005
 epsilon=0.002; %en el paper 0.008


 TF=6;

 t1_ini=1.4941; %para el atractor
 t2_ini=-1.9134; %para el atractor
...
options = simset(simget('Lorenz_attractor_simulink'),'RelTol',1E-6);
exosistema=sim('Lorenz_attractor_simulink',Tf,options);

px(1,:)=0.6+0.007*exosistema.w.signals.values(:,1);
py(1,:)=0.4+0.007*exosistema.w.signals.values(:,3);
...
grid on
% Initialize video
Lorenz = VideoWriter('Lorenz'); %open video file
Lorenz.FrameRate = 100;  %can adjust this, 5 - 10 works well for me
open(Lorenz)

for k = 1:50:length(tiempo)
    addpoints(h,px(k),py(k));
    drawnow
    frame = getframe(gcf); %get frame
    writeVideo(Lorenz, frame);
end
close(Lorenz)
...
%  px=.6; %para el atractor
%  py=.55; %para el atractor
...
%    grid on 
% %   axis equal;
%   plot(tiempo, theta_2b, 'k-')
%     xlabel('tiempo (s)','fontsize',FS)
%   ylabel('\Psi_2 (rad)','fontsize',FS)

  %% generación video presentación
figure(3)
h = animatedline;
axis([0,Tf,0,pi])
grid on
% Initialize video
theta_1 = VideoWriter('theta_1'); %open video file
theta_1.FrameRate = 100;  %can adjust this, 5 - 10 works well for me
open(theta_1)

for k = 1:50:length(tiempo)
    addpoints(h,tiempo(k),theta_1b(k));
    drawnow
    frame = getframe(gcf); %get frame
    writeVideo(theta_1, frame);
end
...
% Initialize video
theta_2 = VideoWriter('theta_2'); %open video file
theta_2.FrameRate = 100;  %can adjust this, 5 - 10 works well for me
open(theta_2)

for k = 1:50:length(tiempo)
    addpoints(h,tiempo(k),theta_2b(k));
%     addpoints(h,t(k),exo(k,1));
%     drawnow
%     addpoints(h,t(k),exo(k,2));
    drawnow
%     pause(0.0005)
    frame = getframe(gcf); %get frame
    writeVideo(theta_2, frame);
...
% Kd=place(Ad,Bd,[0.8 0.85 0.9 0.95]);    %ganancias que estabilizan al sistema
